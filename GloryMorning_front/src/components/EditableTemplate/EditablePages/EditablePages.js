...
class EditablePages extends Component {
      ...
      <div className={style.Container}>
        ...
        <div className={this.state.containerCss}>
          ...
          <MyButton onClick={pushLogout}>
            <FontAwesomeIcon icon="sign-out-alt" size="1x" color="#ced4da" />
            로그아웃
          </MyButton>
        </div>
      </div>
  ...
  handelButton = e => {
    ...
    this.setState({
      button:
        e.currentTarget.id === 'editableButton' ? (
          <div
  ...
  initPages = () => {
    ...
    this.setState({
      pageList: gubunCDPage.map((item, i) => {
        return (
          <div
            className={item === '대시보드' ? style.page_home : style.page}
            onClick={this.props.loadPage}
            key={item}
            id={item}
            ref={ref =>
              (this.item = ref)
            } /* ref를 dataItem 배열인덱스로 잡는다  */
          >
            {item === '대시보드' ? (
              <div className={style.fontAwesomeLogo} id={item}>
                <FontAwesomeIcon
                  icon="home"
                  size="2x"
                  color="#e67700"
